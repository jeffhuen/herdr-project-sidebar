...
use crossterm::{event::{self, DisableMouseCapture, EnableMouseCapture, Event, KeyCode,
    KeyEventKind, KeyModifiers, MouseButton, MouseEventKind}, execute,
    terminal::{disable_raw_mode, enable_raw_mode, EnterAlternateScreen, LeaveAlternateScreen}};
use ratatui::{backend::CrosstermBackend, layout::{Position, Rect}, style::{Modifier, Style},
    widgets::{Paragraph, Wrap}, Terminal};
use crate::config::{self, IconMode, Settings};

pub const LABELS: [&str; 10] = ["Width", "Dock side", "Start open by default", "Order", "Branches", "Task titles",
    "Quiet idle titles", "Agent icons", "Sidebar style", "Install compact font"];
pub const HELP: [&str; 10] = [
    ...
    "Default for new clients. Herdr's remembered manual hide/show choice takes precedence.",
...
pub fn values(settings: &Settings) -> [String; 10] {
    ...
    [format!("[-] {} [+]", settings.width), side(), on(settings.auto_open),
        if settings.grouped { "Projects" } else { "Recent" }.into(),
        on(settings.show_branch), on(settings.show_title), on(settings.dim_idle),
        match settings.icons { IconMode::Text => "Text", IconMode::Font => "Font", IconMode::None => "None" }.into(),
        if settings.project_style { "Projects" } else { "Herdr" }.into(),
        "[Install]".into()]
}

pub fn change(settings: &mut Settings, row: usize, forward: bool) {
    match row {
        0 => settings.width = if forward { settings.width.saturating_add(2).min(80) }
            else { settings.width.saturating_sub(2).max(24) },
        ...
        7 => settings.icons = match (settings.icons, forward) {
    ...
    loop {
        if dirty {
            terminal.draw(|frame| {
                ...
                let height = area.height.saturating_sub(8).max(1).min(LABELS.len() as u16) as usize;
                ...
                for row in offset..(offset + height).min(LABELS.len()) {
                    ...
                    hits[row] = rect;
                    let style = if row == selected { Style::default().add_modifier(Modifier::REVERSED) }
                        else { Style::default() };
                    ...
                    frame.render_widget(Paragraph::new(format!("{} {}", if row == selected { ">" } else { " " }, LABELS[row])).style(style),
                ...
                if y < area.bottom().saturating_sub(2) {
                    let text = if message.is_empty() { HELP[selected] } else { &message };
                    frame.render_widget(Paragraph::new(text).wrap(Wrap { trim: true }),
                        Rect::new(area.x, y, area.width, area.bottom().saturating_sub(y + 2)));
                }
                if area.height > 1 {
                    frame.render_widget(Paragraph::new("Click to edit | arrows/jk/hl | Esc/s/q close"),
        ...
        if !event::poll(Duration::from_secs(1))? {
            match config::load() {
                ...
                Err(error) => { let text = error.to_string(); dirty |= text != message; message = text; }
                _ => {}
            }
            continue;
        }
                ...
                match mouse.kind {
                    ...
                    MouseEventKind::Down(MouseButton::Left) => {
                        if let Some(row) = hits.iter().position(|hit| hit.contains(point)) {
                            selected = row;
                            if row != 0 { edit = Some(true); }
                            else if less.contains(point) { edit = Some(false); }
                            else if more.contains(point) { edit = Some(true); }
                        }
                    }
        ...
        if selected != previous { message.clear(); dirty = true; }
        if let Some(forward) = edit {
            let result = if selected == 9 {
                ...
                config::update(|current| change(current, selected, forward)).and_then(|saved| {
                    if selected == 1 {
                        crate::dock::migrate_open_docks(saved.dock_right, saved.width);
                    }
                    settings = saved;
                    crate::reload().map_err(|error| io::Error::other(format!("Saved; Herdr reload failed: {error}")))?;
                    crate::native::start()?;
                    Ok(if selected == 2 { "Saved startup default. Herdr remembers manual visibility changes." }
                        else { "Saved." }.to_owned())
                })
            };
