...
fn reload() -> io::Result<()> {
    ...
    else { Err(io::Error::other(format!("Herdr configuration reload did not fully apply: {result}"))) }
}

fn main() -> io::Result<()> {
    match std::env::args().nth(1).as_deref() {
        None | Some("dock" | "--dock") => dock::run(),
        Some("--toggle" | "toggle") => dock::toggle(),
        Some("--ensure" | "ensure") => dock::ensure(),
        Some("--dump-snapshot" | "--launch-decision") => dock::run(),
        Some("--configure") => {
            config::configure()?;
            reload()?;
            native::start()
        ...
        Some("--settings") => ipc::call("plugin.pane.open", json!({
        ...
        Some("--help" | "-h") => {
            println!("herdr-project-sidebar\n\
                Terminal dock mode:\n\
                    (no args)       Run interactive project sidebar dock\n\
                    --toggle        Toggle or focus dock in current tab\n\
                    --ensure        Ensure dock pane exists without stealing focus\n\
                Native sidebar mode:\n\
        ...
        Some(flag) => Err(io::Error::new(io::ErrorKind::InvalidInput,
